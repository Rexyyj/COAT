...
SRC_HOST="vm2" # Migration source host name
DST_HOST="vm"  # Migration destination host name
...
initHeaderCSV_iperf () {
    echo "iter,socket,mss,start,end,bps" > $1
}
...
initHeaderCSV_iperf ./log/$TEST_CASE/iperf.csv
initHeaderCSV_time ./log/$TEST_CASE/iperf_op_time.csv
...
sshpass -p $SSHPASS ssh -t $DST_HOST "sudo tc qdisc add dev ens3 root tbf rate 1000mbit latency 50ms burst 1000k"
sshpass -p $SSHPASS ssh -t $SRC_HOST "sudo tc qdisc add dev ens3 root tbf rate 100mbit latency 50ms burst 100k"
sudo tc qdisc add dev ens3 root tbf rate 100mbit latency 50ms burst 100k

for iter in {1..50}
do
    echo "---------------iter $iter----------------------"
    # $SRC_HOST: run sockperf server
    sshpass -p $SSHPASS ssh -t $SRC_HOST "sudo $REPO_DIR_SRC/base_scripts/namespace.sh isrv 172.16.0.10/12 && sudo podman run -d --rm --runtime /usr/bin/runc --name=server --network ns:/run/netns/isrv iperf -s"
    echo "---------------iperf server container created------------------------------" 
    ...
    sudo podman run -d --rm -v $REPO_DIR_CLI/logs:/log --name=client --privileged --network ns:/run/netns/icli iperf -c 172.16.0.10 -t 20 -M 1000 -i 0.1 -R -J --logfile '/log/iperf.log'
    echo "---------------iperf client container created------------------------------"
    sleep 3
    ...
    perf stat -o "$REPO_DIR_CLI/logs/ns_clear.log" sshpass -p $SSHPASS ssh -t $SRC_HOST "perf stat -o $REPO_DIR_SRC/logs/ns_clear_real.log sudo $REPO_DIR_SRC/base_scripts/clear_namespace.sh isrv && sudo chown cal-admin $REPO_DIR_SRC/checkpoints/ckpt.tar.gz"
    ...
    fromTimeLogstoCSV ./log/$TEST_CASE/iperf_op_time.csv $iter
    fromStateLogstoCSV ./log/$TEST_CASE/container_state.csv $iter
    python3 ./iperf2csv.py $REPO_DIR_CLI/logs/iperf.log ./log/$TEST_CASE/iperf.csv $iter
